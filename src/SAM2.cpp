...
std::variant<bool,std::string> SAM2::inference(cv::Mat &image){
    ...
    auto& img_encoder_out =  std::get<0>(result_0); // pix_feat,vision_feats,vision_pos_embed
    std::print("image encoder ok!");
    //******************************mem_attention*********************************
    auto result_1 = this->mem_attention_infer(img_encoder_out);  // mem_attention_in(vision_feats,vision_pos_embed)
    if(result_1.index() != 0) return std::get<std::string>(result_1);
    auto& mem_attention_out = std::get<0>(result_1); // image_embed
    std::print("mem attention ok!");
    //*****************************img_decoder**********************************
    auto result_2 =this->img_decoder_infer(mem_attention_out, infer_status.current_frame != 0);
    if(result_2.index() != 0) return std::get<std::string>(result_2);
    auto& img_decoder_out = std::get<0>(result_2); // obj_ptr,mask_for_mem,pred_mask，object_score_logits
    std::print("image decoder ok!");
    //***********************************************************************
...
//      vision_feats    [1,256,32,32]
//      vision_pos_embed [1024,1,256]
...
std::variant<std::vector<Ort::Value>,std::string> SAM2::mem_attention_infer(std::vector<Ort::Value>&img_encoder_out){
    ...
    if(infer_status.current_frame == 0) [[unlikely]]{
        mem_attention_out.push_back(std::move(img_encoder_out[1])); // vision_feats
    ...
    input_tensor.push_back(std::move(img_encoder_out[1])); //current_vision_feat
    input_tensor.push_back(std::move(img_encoder_out[2])); //current_vision_pos_embed
    ...
    std::vector<float> maskmem_features_(features_size*64*32*32);
    for(size_t i = 0;i<features_size;i++){
        ...
        std::copy_n(tensor_data, 64*32*32, std::begin(maskmem_features_)+64*32*32*i);
    }
    std::vector<int64_t> dimensions_1{(int64_t)features_size,64,32,32}; // [x,64,32,32]
    ...
    size_t maskmem_pos_enc_size = (maskmem_buffer_size*1024+4*std::min(infer_status.current_frame,16))*64;
    
    std::vector<float> maskmem_pos_enc_(maskmem_pos_enc_size);
    
    // a[] , b[1024,1,64], c[1,1,64]
    ...
    for(size_t j = 0;j<maskmem_buffer_size;j++){
        ...
        auto sub = temp_tensor[0].GetTensorData<float>();//[1024,1,64]
        float* p = maskmem_pos_enc_.data() + j*1024*64;
        tensor_add(p,sub,temporal_code.at(j)); // [1024,1,64] + [1,1,64] ->[1024,1,64] + [1024,1,64] ->[1024,1,64]
    }
    // 第二部分：
   std::fill_n(maskmem_pos_enc_.begin()+maskmem_buffer_size*1024*64,maskmem_pos_enc_size - (maskmem_buffer_size*1024*64), 0.0f);
    
    // [z,1,64]
    std::vector<int64_t> dimensions_3{int64_t(maskmem_buffer_size*1024+4*std::min(infer_status.current_frame,16)),1,64};
...
std::variant<std::vector<Ort::Value>,std::string> SAM2::img_decoder_infer(std::vector<Ort::Value>&mem_attention_out, bool no_add_prompt=true){
    std::vector<const char*> input_names,output_names;
    ...
    // point_coords,point_labels,frame_size,image_embed
    std::vector<Ort::Value> input_tensor; // 4
    auto box = parms.prompt_box;
    auto point = parms.prompt_point;
    // 变化bbox比例
    box.x = 512*((float)box.x / ori_img->cols);
    box.y = 512*((float)box.y / ori_img->rows);
    box.width = 512*((float)box.width / ori_img->cols);
    box.height = 512*((float)box.height / ori_img->rows);
    point.x = 512*((float)point.x / ori_img->cols);
    point.y = 512*((float)point.y / ori_img->rows);
...
// input:
//      mask_for_mem    [1,1,512,512]
//      pix_feat        [1024,1,256]
//      object_score_logits [1,1] 
// output:
//      maskmem_features  [1,64,32,32]
//      maskmem_pos_enc   [1024,1,64]
